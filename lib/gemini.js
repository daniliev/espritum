...
// 429 exclu volontairement : c'est un quota, pas un incident. Retenter le
// consomme trois fois plus vite — on remonte l'information à l'utilisateur.
const RETRYABLE = [500, 502, 503, 504];
// Modèle changeable sans toucher au code : si celui-ci est saturé côté Google,
// il suffit de poser GEMINI_MODEL dans les variables Vercel et de redéployer.
...
export async function callGemini(key, payload, opts) {
  ...
  // reason distingue nos propres coupures des vrais refus de Google : sans ça
  // un modèle simplement lent était rapporté comme « surchargé ».
  const msg = (last.data && last.data.error && last.data.error.message) || '';
  const quota = last.status === 429;
  // Google indique combien de temps attendre : « Please retry in 12.5s »
  let retryAfter = null;
  ...
  return {
    ...
    status: last.status,
    data: last.data,
    reason: last.status === 0 ? (msg === 'timeout' ? 'timeout' : 'network') : ('upstream_' + last.status),
    quota: quota,
